...
const itemCountValueElement = document.getElementById('itemCountValue');
...
ui.on_message('item_count', message => {
  updateItemCount(message);
});
...
// Updates the "Item Count: N" badge. `message` is expected to be the
// current total count sent from the Python side each time an item is
// picked up (see sweep_brush() call site in main.py).
function updateItemCount(message) {
  if (itemCountValueElement) {
    itemCountValueElement.textContent = message;
  }
}
